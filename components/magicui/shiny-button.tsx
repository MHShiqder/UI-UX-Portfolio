...
// Use a typed object for CSS variables
const animationProps: MotionProps = {
  initial: { ["--x" as string]: "100%", scale: 0.8 } as Record<string, string | number>,
  animate: { ["--x" as string]: "-100%", scale: 1 } as Record<string, string | number>,
  whileTap: { scale: 0.95 },
  transition: {
    repeat: Infinity,
    repeatType: "loop",
...
export const ShinyButton = React.forwardRef<HTMLButtonElement, ShinyButtonProps>(
  ({ children, className, ...props }, ref) => {
    return (
      ...
      >
        <span
          className="relative block w-full text-sm uppercase tracking-wide text-[rgba(0,0,0,0.65)] dark:font-light dark:text-[rgba(255,255,255,0.9)]"
          style={{
            maskImage:
